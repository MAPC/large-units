d3.csv("/large-units/assets/data/largeunits-hhtype.csv").then((rawData) => {
  ...
  const formattedData = seriesGen(rawData
    .filter((type) => type.type === "All Large Units")
    .map((item, i) => {
      return {
        type: item.type,
  ...
  const margin = {top: 80, right: 0, bottom: 40, left: 40}
  , width = 400
  , graphWidth = width - margin.left
  , canvasHeight = 420
  , graphHeight = canvasHeight - margin.top - margin.bottom;

  const svg = d3.select(".largeunits_hhtype_allunits-frame")
    .attr("width", width)
  ...
  graph.selectAll("g.household-type")
    ...
    addLegend(margin.top + graphHeight, colors);
})
